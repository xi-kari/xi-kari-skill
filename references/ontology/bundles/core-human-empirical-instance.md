# Core-human bundle: human empirical instance closure

## 联读目的

把通用根实例资格与 H1、H4、H5 的独立经验实例合同闭合起来，防止把抽象 H 命题、接口字段或内部定义误作外部经验支持。
...
- 不能只读 H1、H4、H5 名称就宣称经验成立。
- 不能把根实例合同直接改名为 H-instance；身份域必须从 `root_id` 切换为只允许 H1/H4/H5 的 `claim_id`。
- H-instance 即使 `supported` 也不产生价值、责任、义务、正当性、授权或现实处置。
- H2、H3、H6 是分类或规范边界，不进入这份经验实例合同。

## 闭包条件

读取计划必须同时包含上述 core 与 human 卡；任一 required-neighbor 边缺失、实例冻结不可审计、结果后择优或外部证据制品不可解析时，降回候选或未决。
