...
附录 A 要求每张 HV 卡按 A-E 五区登记同一组 39 个字段（`V83-P2907`-`V83-P2908`、`V83-P1216`-`V83-P1218`）。本仓库的逐卡文件提供人类可读的定义、上限、反例和三阶接口；完整字段仍以 v8.3 reader 的附录 A 为权威，不能用本包的压缩替代原文。
...
- `inferential_requires`、`protocol_requires`、`specializes`、`applies_to` 四类依赖不可混用；只在某条结论路由需要的条件进入 `conditional_support_routes`（`V83-P1217`）。
- 空集合只表示没有登记依赖，不表示证据充分。
- H1/H4/H5 的实例证据、H2/H3/H6 的分类/规范边界和 CM 条件机制不能互相冒充。
- 跨尺度时逐轴登记保持、改变、丢失、低可见位置和局部排除区；判断上限不超过证据，行动上限不超过 J。
- 新变量若没有 v8.3 定义，写入 `source_undefined_fields` 或运行时 `XK-PROV-*`，不得静默升格。

## 覆盖

`hv01-structure-domain.md` 至 `hv11-open-bearing-action.md` 分别对应 HV01-HV11；原始全文位于 `references/source/v8.3/reader/17-appendix-a-human-variable-cards.md`，每张卡必须与相应 `V83-P` 锚点逐项核对。
...
- [v8.3 reader 07](../../source/v8.3/reader/07-human-structured-world.md): `V83-P1215`-`V83-P1244`。
- [v8.3 reader 附录 A](../../source/v8.3/reader/17-appendix-a-human-variable-cards.md): `V83-P2906`-`V83-P4477`。
