---
id: V82-CANON-REVISION-LINEAGE
qualified_id: governance:v8.2-revision-lineage
name: v8.2 版本修订谱系
...
附录 C 要求依 GOV-19 随文档保留修订记录。v8.2 在不删除 v8.1 正文的前提下补入双文本共同内核、表示/表述转义、任务相关损失审计、有效变量与闭合、三阶递归未来推演、局部可预测性和数学准入；没有增加新的经验根假设或第十尺度算子（`V82-P4573`-`V82-P4577`）。

## 允许推论

- 把 v8.2 新增能力作为版本差异和来源谱系读取，不把新增字段伪装成旧版经验根假设。
- 保留原段、增补段、独特限定、反例和例证的关系；后续合并必须逐条登记。
- 在运行时声明使用 v8.2 的 raw/semantic 身份和版本。
...
若快照覆盖删除旧段、无法定位修订来源、把 v8.1/v8.2 混读、或版本身份不一致，停止并要求源闭合。
...
- [v8.2 reader 附录 C](../../../source/v8.2/reader/19-appendix-c-revisions.md): `V82-P4573`-`V82-P4586`。

## 原文定义区段

本卡的权威定义必须与无损阅读版连续联读；以下锚点是定位索引，不是删减后的替代文本。运行时先完整读取 21 卷，再回到这些锚点核对相邻段落、表格和适用边界。

- `V82-P4573`, `V82-P4574`, `V82-P4575`, `V82-P4577`, `V82-P4583`
...
- `V82-CANON-GOV-WEAK`：`references/ontology/cards/governance-self-limit/protection-ai-exit.md`
