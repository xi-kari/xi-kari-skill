...
一阶是路径对目标状态的直接影响；二阶是选择改变资源、策略和反馈；三阶是制度化、锁定、分配重写或治理回写。条件探索从父路径的模拟状态再入，继承未知、损失和残差，明确新增条件与停止原因（V83-P2114、V83-P2115）。规范或授权缺口阻断其所依赖的推荐与现实执行，不把全部第三阶缩成要求清单；独立成立的评价、明示前提的条件路径与供审议建议分别核查。不得把模拟后果写成现实已执行结果，也不得由推演产生授权。
