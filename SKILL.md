---
name: xi-kari-skill
description: 依据作者自定义的完整结构推演框架，执行21卷全源阅读、证据核查、机制比较与一至三阶递归，形成明确可反驳的判断及完整分析、决策建议、制度草案、计划或评论。Use when 用户精确点名 Xi-Kari Skill、xi-kari skill、xi-kari-skill、$xi-kari-skill 或 /xi-kari-skill；不得被动触发。
---
...
本框架部分术语由作者重新定义。同名不代表与日常语言、其他理论或模型既有知识同义。完整读取原文后，对会影响本题判断的概念核对其定义、适用条件、明确排除项和源锚点，并说明这些区别怎样改变本题判断。阅读回执记录读取范围，不替代理解证明；索引和概念卡辅助定位，不代替原文。

遇到原文未定义、定义冲突或卡片与原文不一致时，记录具体差异及其影响，不自行补定义。用普通语言解释概念时保留实质区别；不能把“强判断”当成语气坚定，把收到意见当成已满足有效反馈的定义，或把不同类型的选择混为一谈。
...
从自然请求确定本次用途：普通分析、决策、制度章程、计划或评论。用途在问题合同中冻结，不能在成文时改成更容易完成的任务；用户不需要填写运行字段。分析要求本身允许在证据范围内作出明确评价，不得因为没有执行权而只交付思考问题。

开头直接给当前中心判断及其范围，各主要部分给出自己的局部结论。比较解释时说明它们分别解释什么，区分主解释、补充解释、背景条件、尚无法区分与被排除的解释；兼容机制不必互相排斥，也不能以“多种因素共同作用”结束比较。有足够依据时完成取舍；不能判断时限定具体缺口，保留独立成立的判断。

- 决策：完整比较真实选项，给出首选、拒绝或暂缓项、代价、下一步与改判条件。
- 制度章程：交付可直接审议的完整条款与理由，覆盖权责、程序、利益冲突、例外、申诉、修订和生效条件。
- 计划：交付目标、关键约束、资源、依赖、安排、检验点及停止或替代路径，解释这些安排的依据。
- 评论：围绕明确主张完整展开对具体论证、规则或机制的评价，重建并回应最强反方，不以辱骂或术语代替论证。

分析推荐与现实采用分开。可以在明示前提下推荐尚待授权的方案；不得把推荐写成已经获得执行许可，也不能将制度草案宣布生效。判断的用途和可预见后果按 v8.3 原文审查，公开传播提高审查要求但不自动使所有评论成为强判断。
...
6. 按 [mechanism-and-case.md](protocols/mechanism-and-case.md) 逐项处置简单基线、主解释、最强竞争解释、混合解释和残差解释；适用的完整比较，不适用的说明原因，不为凑数量虚构机制或强制总排序。为实际机制配置明确标型的案例与反例。
...
原文与默认答案都不采用缩略版，不设置默认字数上限。所有已开展的实质分析须进入完整正文；“不改变主结论”“属于次要分支”不能成为删除理由。不同路径的条件、成本承担者、反例、被排除理由与停止原因均须讲清，不能只放附件或留下覆盖标记。提高可读性靠组织主次、具体解释和连贯论证。单条消息容纳不下时按章节连续交付，并标明尚未交付范围；未交齐不得宣称完整。
